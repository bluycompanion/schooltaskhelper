...
- Den deployade releasen blev `/Users/Shared/dev/runtime/schooltaskhelper/releases/20260613-183937`.
...
- 2026-06-13: Fixade Hermes-/B2LeadDev-kompatibel prod-deploy. Tog bort gamla direkta `nohup`-processen på 4320, lät installerad system-LaunchDaemon `com.webhosting.schooltaskhelper.prod` ta över och rättade `service.sh` till exakt `/bin/launchctl` så sudoers-regeln matchar. Justerade live-verifieringen så den kontrollerar build-id i JS-bundlen (React-renderad footer finns inte i rå HTML) och klarar både direkt Node-check och Caddy `handle_path`-prefix. Körde `/Users/Shared/dev/ops/webhosting/scripts/b2leaddev-deploy.sh schooltaskhelper --approve`: backend 14/14, web 6/6, typecheck, prod build, system-service restart och live build-id-verifiering passerade. Aktiv live build: `20260613-190353-8bb419a`, LaunchDaemon pid `56786`.
